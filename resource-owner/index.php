...
<h1>Step 1</h1>
<p>Let's see how authentication works using a Resource Owner grant. For more information about this grant and how it should be used, please see the <a href="https://github.com/joshcanhelp/oauth-grants-in-php/blob/master/resource-owner/README.md" target="_blank">README here</a></p>
<p>First, let's check what's in our PHP session (<code><?php echo SESSION_AUTH_KEY; ?></code> key) to see if we're already logged in:</p>
...
<?php else: ?>
  ...
  <ul>
    <li>
      ...
      This tells the token endpoint what to expect and how to perform the authentication. In this case, it will be:
      <pre><?php echo $ro_post_body[ 'grant_type' ]; ?></pre>
    </li>
    <li>
      <code>scope</code>:
      The scopes that you want to request authorization for. In this case, we want to identify our user so we'll ask for an OIDC scope and an email. If you use an <code>audience</code> parameter to request access to an API, you can add valid API scopes here as well. The complete scope being used for this example is:
  ...
  <p>Before we can authenticate, we need two more things: a username and a password. This is what makes the Resource Owner grant only appropriate for highly-trusted applications.</p>
